...
class MomoController extends Controller
{
    ...
    {
        // MoMo requires a strict raw string order for HMAC SHA256.
        $raw = implode('&', [
    ...
    public function createOrder(Order $order)
    {
        ...
        $resp = Http::timeout(15)->post($this->endpoint(), $payload);
        if (! $resp->successful()) {
            $json = $resp->json();
    ...
    public function handleReturn(Request $request, Order $order)
    {
        ...
        // Return URL should prioritize user experience; IPN is the authoritative server-to-server channel.
        if ($resultCode === 0) {
